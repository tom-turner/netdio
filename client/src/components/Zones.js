	...
	return(
			<div className={`w-full p-4 space-y-4 rounded ${ selected ? 'border-neutral-500 border shadow-sm' : 'border border-neutral-200 shadow-lg' }`} >
				...
				<div className="flex justify-between w-full space-x-4 ">
			 		...
			 		<div className={`w-full ${ selected ? '' : 'hidden'}`}>
						<select className="text-right text-base my-auto text-neutral-800 w-full" onChange={ (e) => { handleInputChange({ ip: receiver.ip , value: e.target.value }); setSelected({}) } } defaultValue={'default'} >
