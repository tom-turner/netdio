...
class Roc {
  ...
  rocRecv(data) {
    ...
    let rocRecv = spawn('roc-recv', ['-vv', '-s' ,`rtp+rs8m::${source}`, '-r', `rs8m::${getRepairPort(source)}`, outputDriver, outputDevice, `--sess-latency=${latency}`,`--resampler-profile=${profile}`, poisoning, resampling]);
  ...
  rocSend(data) {
    ...
    let rocSend = spawn('roc-send', ['--nbsrc=10', '--nbrpr=5', '-vv', '-s', `rtp+rs8m:${recv}:${source}`, '-r', `rs8m:${recv}:${getRepairPort(source)}`, inputDriver, inputDevice, `--resampler-profile=${profile}`, poisoning, resampling]);
