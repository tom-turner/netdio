...
class Roc {
  constructor(config) {
    ...
    this.outputDriver = this.config.rx.driver ? `-d${this.config.rx.driver}` : ''
    this.outputDevice = this.config.rx.hardware ? `-i${this.config.rx.hardware}` : ''
  }
  ...
  rocRecv(data) {
    ...
    let rocRecv = spawn('roc-recv', ['-vv', '-s' ,`rtp+rs8m::${source}`, '-r', `rs8m::${getRepairPort(source)}`, this.outputDriver, this.outputDevice, resampling, `--sess-latency=${latency}`,`--resampler-profile=${profile}`, poisoning]);
  ...
  rocSend(data) {
    ...
    let inputDevice = data.txdata.tx.hardware ? "-o" + data.txdata.tx.hardware : ""
