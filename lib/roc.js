...
const interval = 100
const latency = '75ms'
const profile = 'high' // high(slow) - medium - low(fast) 
let poisoning = true 
  ? '--poisoning' 
  : ''
let inputDriver = platform.inputDriver() ? `-d ${platform.inputDriver()}` : ''
let outputDriver = platform.outputDriver() ? `-d ${platform.outputDriver()}` : ''
let inputDevice = platform.inputDevice() ? `-i ${platform.inputDevice()}` : ''
let outputDevice = platform.outputDevice() ? `-o ${platform.outputDevice()}` : ''
...
class Roc {
  ...
  rocRecv(data) {
    ...
    let rocRecv = spawn('roc-recv', ['-vv', '-s' ,`rtp+rs8m::${source}`, '-r', `rs8m::${getRepairPort(source)}`, outputDriver, outputDevice, `--sess-latency=${latency}`,`--resampler-profile=${profile}`, poisoning]);
    
    console.log('roc-recv', '-vv', '-s' ,`rtp+rs8m::${source}`, '-r', `rs8m::${getRepairPort(source)}`, outputDriver, outputDevice)
  ...
  rocSend(data) {
    ...
    let rocSend = spawn('roc-send', ['--nbsrc=10', '--nbrpr=5', '-vv', '-s', `rtp+rs8m:${recv}:${source}`, '-r', `rs8m:${recv}:${getRepairPort(source)}`, inputDriver, inputDevice, `--resampler-profile=${profile}`, poisoning]);

    console.log('roc-send', '--nbsrc=10', '--nbrpr=5', '-vv', '-s', `rtp+rs8m:${recv}:${source}`, '-r', `rs8m:${recv}:${getRepairPort(source)}`, inputDriver, inputDevice)
