...
class Roc {
  constructor(config, updateInterval) {
    ...
    this.outputDevice = this.config.rx.hardware ? `-o${this.config.rx.hardware}` : ''
  }
  ...
  rocRecv(data) {
    ...
    let rocRecv = spawn('roc-recv', ['-vv', '-s' ,`rtp+rs8m::${source}`, '-r', `rs8m::${getRepairPort(source)}`, this.outputDriver, this.outputDevice, resampling, latency, profile, poisoning]);
  ...
  rocSend(data) {
    ...
    let rocSend = spawn('roc-send', ['--nbsrc=10', '--nbrpr=5', '-vv', '-s', `rtp+rs8m:${recv}:${source}`, '-r', `rs8m:${recv}:${getRepairPort(source)}`, inputDriver, inputDevice, resampling, latency, profile, poisoning]);
