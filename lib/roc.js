...
function startRx(config) {
  ...
  var rocRecv = spawn('roc-recv', ['-vv', '-s' ,`rtp+rs8m::${sourcePort}`, '-r', `rs8m::${repairPort}`, driver, `--sess-latency=${latency}`,`--resampler-profile=${profile}`, poisoning]);
...
function startTx(config) {
  ...
  var rocSend = spawn('roc-send', ['--nbsrc=10', '--nbrpr=5', '-vv', '-s', `rtp+rs8m:${multicastIp}:${sourcePort}`, '-r', `rs8m:${multicastIp}:${repairPort}`, driver, `--resampler-profile=${profile}`, poisoning]);
