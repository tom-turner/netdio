...
const latency = '--sess-latency=500ms' // --sess-latency=95ms
...
class Roc {
  ...
  rocSend(data) {
    ...
    let rocSend = spawn('roc-send', ['--nbsrc=10', '--nbrpr=5', '-vv', '-s', `rtp+rs8m:${recv}:${source}`, '-r', `rs8m:${recv}:${getRepairPort(source)}`, inputDriver, inputDevice, rate, resampling, profile, poisoning]);
