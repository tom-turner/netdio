...
class Roc {
  ...
  startRocSend(data) {
    ...
    this.timeout((dead)=>{
      dead ? this.kill([rocSend.pid]) : true
    }) 
    ...
    let source = data ? data.source : this.config.tx.source
    let ip = data ? data.ip : ''

    let rocSend = spawn('roc-send', ['--nbsrc=10', '--nbrpr=5', '-vv', '-s', `rtp+rs8m:${ip}:${source}`, '-r', `rs8m:${ip}:${getRepairPort(source)}`, driver, `--resampler-profile=${profile}`, poisoning]);
