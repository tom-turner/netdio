...
function startRx(config) {
  ...
  var rocRecv = spawn('roc-recv', ['-vv', '-s' ,`rtp+rs8m::${sourcePort}`, '-r', `rs8m::${repairPort}`, driver, '--sess-latency=50ms','--resampler-profile=low']);
...
function startTx(config) {
  ...
  var rocSend = spawn('roc-send', ['--nbsrc=20', '--nbrpr=10', '-vv', '-s', `rtp+rs8m:${multicastIp}:${sourcePort}`, '-r', `rs8m:${multicastIp}:${repairPort}`, driver, '--resampler-profile=low', 'packet-length=123h']);
