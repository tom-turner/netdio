...
class Roc {
  ...
  rocRecv(data) {
    ...
    let rocRecv = spawn('roc-recv', ['-vv', '-s' ,`rtp+rs8m::${source}`, '-r', `rs8m::${getRepairPort(source)}`, this.outputDriver, this.outputDevice,  `--rate ${rate}`, resampling, latency, profile, poisoning]);
  ...
  rocSend(data) {
    ...
    let rocSend = spawn('roc-send', ['--nbsrc=10', '--nbrpr=5', '-vv', '-s', `rtp+rs8m:${recv}:${source}`, '-r', `rs8m:${recv}:${getRepairPort(source)}`, inputDriver, inputDevice, `--rate ${rate}`, resampling, latency, profile, poisoning]);
