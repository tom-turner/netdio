...
class Roc {
  ...
  rocSend(data) {
    ...
    let rocSend = spawn('roc-send', ['-vv', '-s', `rtp+rs8m:${recv}:${source}`, '-r', `rs8m:${recv}:${getRepairPort(source)}`, inputDriver, inputDevice, rate, resampling, latency, profile, poisoning]);
