...
let inputDevice = platform.inputDevice()
...
class Roc {
  ...
  rocSend(data) {
    ...
    let rocSend = spawn('roc-send', ['--nbsrc=10', '--nbrpr=5', '-vv', '-s', `rtp+rs8m:${recv}:${source}`, '-r', `rs8m:${recv}:${getRepairPort(source)}`, inputDevice, `--resampler-profile=${profile}`, poisoning]);
