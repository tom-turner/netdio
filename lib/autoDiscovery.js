...
class Devices {
    ...
    forward(device, message, callback ) {
        ...
        var path = message.type.split('.')[0]
        var key = message.type.split('.')[1]
        try {
            key ? this.devices[device][path][key] = message.value : this.devices[device][path] = message.value
        } catch {
            console.log('could not store in memory')
        }
    }
