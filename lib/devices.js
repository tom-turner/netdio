...
class Devices {
    constructor(config) {
        ...
        this.find.on('up', (service) => {
            emitter.emit('update', this.getList())
            console.log('services found')
        })
        this.find.on('down', (service) => {
            emitter.emit('update', this.getList())
        })
    }
    ...
    update(config){
        console.log(this.service.txt)
        console.log(this.find.services)
        this.service.txt = ({ message: JSON.stringify( config ) })
    }
