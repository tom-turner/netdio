    ...
    announce(devices){
        for (var device of devices) {
            ...
            this.forward('discovery', device.name, 'keep alive', (res) => {
                if (res.success) {
                    console.log(3, device.name)
                    console.log(4, this.devices)
                    this.addDevice(device.name, res.device)
                }
                if (res.error) {
                    try {
                        //console.log('device down', device.name)
                        //this.removeDevice(device.name)
