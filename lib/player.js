...
class Player {
	...
	start(service){
		if(service == 'spotify') {	
			...
			let spotify = exec('~/librespot/target/release/librespot -n Duck -b 320 --initial-volume 95 --enable-volume-normalisation --normalisation-pregain -3 --device hw:Loopback,0')
			...
			spotify.stderr.on('data', (data) => {
				...
				let id = data.match(/<spotify:track:(.*?)>/)[1]
