...
class Player {
	...
	start(service){
		if(service == 'spotify') {	
			...
			let spotify = exec(`~/librespot/target/release/librespot -n Duck-${number} -b 160 --initial-volume 95 --enable-volume-normalisation --normalisation-pregain -3 --device 'hw:Loopback,1'`)
