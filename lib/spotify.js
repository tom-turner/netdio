...
class Spotify {
	...
	startAndKeepAlive(id, callback){
		...
		this.spotify = exec(`~/librespot/target/release/librespot -n Duck-${this.config.device.name} -b 160 --initial-volume 95 --enable-volume-normalisation --normalisation-pregain -3 ${this.output()}`)
